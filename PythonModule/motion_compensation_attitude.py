#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""UAV-GPR 姿态/APC 足迹修正模块。

默认使用 V2 风格的核心逻辑，同时保留 V1 实现用于兼容性测试。
"""
...
from core.motion_compensation_core import (
    apply_attitude_apc_correction,
    _clone_metadata,
    _field_1d,
    _select_height,
    _compute_trace_distance,
    _rotate_xy,
    _warning,
)
...
REQUIRED_FIELDS_V1 = ("roll_deg", "pitch_deg", "yaw_deg", "local_x_m", "local_y_m")


def _finite_float_v1(value: object, name: str) -> float:
    try:
        parsed = float(value)
...
def _positive_float_v1(value: object, name: str) -> float:
    parsed = _finite_float_v1(value, name)
    if parsed <= 0:
        raise ValueError(f"{name} 必须为正数")
    return parsed


def _extract_numeric_field_v1(
    trace_metadata: dict,
    key: str,
    trace_count: int,
) -> np.ndarray:
...
def _compute_trace_distance_v1(local_x_m: np.ndarray, local_y_m: np.ndarray) -> np.ndarray:
    if local_x_m.size == 0:
...
def _apply_yaw_rotation_v1(
    x_body_m: np.ndarray, y_body_m: np.ndarray, yaw_rad: np.ndarray
...
def method_motion_compensation_attitude_v1(
    data: np.ndarray,
    ...
    **kwargs,
    ...
    apc_offset_x_m = _finite_float_v1(apc_offset_x_m, "apc_offset_x_m")
    apc_offset_y_m = _finite_float_v1(apc_offset_y_m, "apc_offset_y_m")
    apc_offset_z_m = _finite_float_v1(apc_offset_z_m, "apc_offset_z_m")
    max_abs_tilt_deg = _positive_float_v1(max_abs_tilt_deg, "max_abs_tilt_deg")
    meta: dict[str, object] = {
        "method": "motion_compensation_attitude_v1",
        ...
        "provenance": {
            ...
            "required_fields": list(REQUIRED_FIELDS_V1),
        },
    }
    ...
    missing_fields = [
        field for field in REQUIRED_FIELDS_V1 if field not in trace_metadata
    ]
    if missing_fields:
        meta["skipped"] = True
        meta["missing_fields"] = missing_fields
        meta["reason"] = (
            f"缺少姿态/APC 修正所需字段: {', '.join(missing_fields)}"
        )
        return amplitude_out, meta

    try:
        local_x_m = _extract_numeric_field_v1(trace_metadata, "local_x_m", trace_count)
        local_y_m = _extract_numeric_field_v1(trace_metadata, "local_y_m", trace_count)
        roll_deg = _extract_numeric_field_v1(trace_metadata, "roll_deg", trace_count)
        pitch_deg = _extract_numeric_field_v1(trace_metadata, "pitch_deg", trace_count)
        yaw_deg = _extract_numeric_field_v1(trace_metadata, "yaw_deg", trace_count)
    ...
    if "flight_height_m" in trace_metadata:
        try:
            flight_height_m = _extract_numeric_field_v1(
                trace_metadata, "flight_height_m", trace_count
            )
        except ValueError as exc:
            meta["skipped"] = True
            meta["reason"] = str(exc)
        ...
        if np.any(flight_height_m <= 0.0):
            meta["skipped"] = True
            meta["reason"] = (
                "trace_metadata['flight_height_m'] 包含零或负值，无法构造有效足迹几何"
            )
            return amplitude_out, meta
    ...
    clamped_mask = (np.abs(roll_deg) > tilt_limit_deg) | (
        np.abs(pitch_deg) > tilt_limit_deg
    )
    ...
    apc_local_x_m, apc_local_y_m = _apply_yaw_rotation_v1(
        apc_body_x, apc_body_y, yaw_rad
    )
    ...
    if flight_height_m is None:
        ...
        if np.any(projection_height_m <= 0.0):
            meta["skipped"] = True
            meta["reason"] = (
                "flight_height_m 与 apc_offset_z_m 组合后非正，无法构造有效投影高度"
            )
            return amplitude_out, meta
    ...
    footprint_dx_m, footprint_dy_m = _apply_yaw_rotation_v1(
        pitch_body_m, roll_body_m, yaw_rad
    )

    corrected_local_x_m = local_x_m + apc_local_x_m + footprint_dx_m
    corrected_local_y_m = local_y_m + apc_local_y_m + footprint_dy_m
    trace_distance_m = _compute_trace_distance_v1(corrected_local_x_m, corrected_local_y_m)
    ...
    meta["projection_height_min_m"] = (
        float(np.min(projection_height_m)) if projection_height_m.size else 0.0
    )
    meta["projection_height_max_m"] = (
        float(np.max(projection_height_m)) if projection_height_m.size else 0.0
    )
...
def method_motion_compensation_attitude(
    ...
    **kwargs,
    ...
    """姿态/APC 足迹修正（使用 V2 风格核心逻辑）。

    默认使用 V2 风格的核心逻辑（优先使用 height_agl_m）。

    Args:
        data: 输入 B-scan 数据
        apc_offset_x_m: APC X 偏移（米）
        apc_offset_y_m: APC Y 偏移（米）
        apc_offset_z_m: APC Z 偏移（米）
        max_abs_tilt_deg: 最大允许倾角（度）
    ...
    if trace_metadata is None:
        ...
        # If we only have flight_height_m (not height_agl_m), or missing attitude fields, use V1 for compatibility
    ...
    if use_v1:
        return method_motion_compensation_attitude_v1(
            data=data,
            apc_offset_x_m=apc_offset_x_m,
            apc_offset_y_m=apc_offset_y_m,
            apc_offset_z_m=apc_offset_z_m,
    ...
    corrected, meta = apply_attitude_apc_correction(
        data=data,
        trace_metadata=trace_metadata,
        apc_offset_x_m=apc_offset_x_m,
        apc_offset_y_m=apc_offset_y_m,
    ...
    meta["method"] = "motion_compensation_attitude"

    # Backward compatibility keys
    meta["apc_offset_x_m"] = apc_offset_x_m
    meta["apc_offset_y_m"] = apc_offset_y_m
    meta["apc_offset_z_m"] = apc_offset_z_m
    meta["max_abs_tilt_deg"] = max_abs_tilt_deg
