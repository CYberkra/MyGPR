...
# Reduce native-startup instability on Windows: repeated findfont warnings can
# flood launcher logs and obscure the real error.  Font selection is handled by
# ui.matplotlib_fonts after Matplotlib imports.
...
class GPRGuiQt(MainWindowDataLoadingMixin, MainWindowAutoTuneStartMixin, MainWindowExportMixin, MainWindowDisplayMixin, MainWindowQualityMixin, MainWindowWorkerMixin, QMainWindow):
    ...
    def closeEvent(self, event):
        ...
        for child_canvas in list(self.findChildren(FigureCanvas)):
            try:
                child_canvas.figure.clear()
                plt.close(child_canvas.figure)
            except Exception:
                pass
            try:
                child_canvas.close()
                child_canvas.deleteLater()
            except Exception:
                pass
    ...
    def _init_keyboard_shortcuts(self) -> None:
        """注册键盘快捷键（带 hasattr 守卫，不存在的槽自动跳过）。"""
        # ── 页面导航 Ctrl+1..5 ──────────────────────────────────
        if hasattr(self, "_switch_side_workspace"):
            QShortcut(QKeySequence("Ctrl+1"), self).activated.connect(
                lambda: self._switch_side_workspace(0)
            )
            QShortcut(QKeySequence("Ctrl+2"), self).activated.connect(
                lambda: self._switch_side_workspace(1)
            )
            QShortcut(QKeySequence("Ctrl+3"), self).activated.connect(
                lambda: self._switch_side_workspace(2)
            )
            QShortcut(QKeySequence("Ctrl+4"), self).activated.connect(
                lambda: self._switch_side_workspace(3)
            )
            QShortcut(QKeySequence("Ctrl+5"), self).activated.connect(
                lambda: self._switch_side_workspace(4)
            )

        # ── 执行处理 ─────────────────────────────────────────────
        if hasattr(self, "apply_method_from_selected_source"):
        ...
        # ── 重置为原始数据 ────────────────────────────────────────
        if hasattr(self, "reset_original"):
            QShortcut(QKeySequence("Ctrl+Shift+Z"), self).activated.connect(
                self.reset_original
            )

        # ── 导入数据 ─────────────────────────────────────────────
        if hasattr(self, "import_csv_file"):
        ...
        if hasattr(self, "_refresh_plot"):
            QShortcut(QKeySequence("F5"), self).activated.connect(self._refresh_plot)
    ...
    def _setup_ui(self):
        ...
        idx_auto_tune = self.control_tabs.addTab(
            self.page_auto_tune, FluentIcon.SETTING.icon(), "参数推荐"
        )
        self.control_tabs.setTabToolTip(idx_auto_tune, "自动生成处理参数、查看候选对比和推荐报告")
        ...
        self._page_quality_nav = self._create_workspace_nav_placeholder(
            title="质量与报告页面",
            body="质量概览、航迹检查、报告导出和处理记录会在主视图显示。",
        ...
        self._page_terrain_nav = self._create_workspace_nav_placeholder(
            title="空间成果页面",
            body="地形剖面、三维地理参考、C-scan 和基覆界面会在主视图显示。",
        ...
        self._plot_coord_label.setToolTip("鼠标悬停时显示道号、采样点和振幅。")
    ...
    def _create_workspace_nav_placeholder(self, title: str, body: str, action: str) -> QWidget:
        ...
        action_label = QLabel("主视图已切换。本区域仅保留快捷入口。")
    ...
    def switch_to_main_mode(self, tab_key: str | None = None):
        ...
        if self._content_stack is not None and self._main_content_widget is not None:
            ...
            if tab_key == "basic" and self.page_basic is not None:
                ...
                self.status_label.setText("参数推荐")
    ...
    def switch_to_workbench_mode(self):
        ...
        self.switch_to_main_mode("basic")
        self._log("旧工作台已移除；请使用日常处理、参数推荐、显示与对比、质量与导出、地形/三维成果五个主标签页。")
    ...
    def _on_shared_data_changed(self, payload: dict):
        ...
        if reason in {"loaded", "undo", "reset"}:
            ...
            if reason == "loaded":
                self._no_prior_guard_events = []
            self._reset_auto_tune_state("数据已更新，请重新生成参数推荐。")
    ...
    def _record_structured_error(
        ...
        *,
        ...
        """Normalize an exception to structured export metadata."""
    ...
    def _get_structured_runtime_log_payload(self, timestamp: str) -> dict:
        """Return structured log payload for exported packages."""
        events = []
    ...
    def _update_interaction_mode_status(self, *args):
        ...
        if self._is_manual_roi_pick_enabled():
            text, tone = "范围", "warning"
            tip = "关注范围框选已开启：按住左键拖动，Esc 可取消临时框。"
    ...
    def apply_method_auto_tuned_default(self):
        ...
        if method_key != self._last_auto_tune_result.get("method_key"):
            self._reset_auto_tune_state("当前方法已变化，请先重新生成参数推荐。")
            self.page_basic.set_apply_source_hint(
    ...
    def _flush_auto_tune_progress_update(self):
        """Apply the latest coalesced recommendation progress payload to widgets."""
    ...
    def _on_auto_tune_finished(self, result: dict):
        ...
        self._set_busy(
            False, text="参数推荐完成" if not cancelled else "参数推荐已取消"
        )
        if cancelled:
            self._pending_apply_after_auto_tune = False
            self.page_basic.set_apply_source_hint(
        ...
        self.page_basic.set_apply_source_hint(
            "已生成推荐参数，可在“应用方法”右侧切换默认应用来源。"
        )
        self.page_auto_tune.show_result(result)
        self._log(
            f"参数推荐完成: {result.get('method_name', result.get('method_key'))} | 推荐参数 {result.get('recommended_params') or result.get('best_params')}"
        )
        self._log_auto_tune_recommendation_label(result)
        if pending_apply:
            self._pending_apply_after_auto_tune = False
            profile_key = str(result.get("recommended_profile", "balanced"))
            self._log(f"参数推荐完成后自动应用推荐档：{profile_key}")
            self.apply_method_from_profile(profile_key)

    def _on_auto_tune_error(self, error_msg: str):
        """参数推荐失败。"""
        self._set_busy(False, text="参数推荐失败")
        self._pending_apply_after_auto_tune = False
        self.page_basic.set_apply_source_hint("自动分析失败，未执行方法。")
        self.page_auto_tune.show_error(error_msg)
        self._log(f"参数推荐失败: {error_msg}")
        QMessageBox.warning(self, "参数推荐失败", error_msg)
    ...
    def _set_auto_tune_comparison_snapshots(self, result):
        ...
        manual_label = next((label for label in labels if label.startswith("手动参数")), "")
        auto_label = next((label for label in labels if label.startswith("参数推荐")), "")
    ...
    def apply_method_from_profile(self, profile_key: str):
        ...
        if method_key != self._last_auto_tune_result.get("method_key"):
            ...
            self._reset_auto_tune_state("当前方法已变化，请重新生成参数推荐。")
            return
        ...
        self.page_basic.set_apply_source_hint(
            f"将使用推荐参数 - {profile.get('label', profile_key)}"
        )
        self._log(f"使用推荐参数{profile.get('label', profile_key)}执行当前方法。")
        self.apply_method()

    def show_auto_tune_details(self):
        """显示参数推荐候选评分详情。"""
        if not self._last_auto_tune_result:
            QMessageBox.information(self, "参数推荐", "暂无候选评分结果。")
            return
    ...
    def import_gprmax_out_file(self):
        ...
        if not path:
            return
        self._load_with_progress("加载 .out 数据文件", self._load_gprmax_out, path)
...
def create_main_window(version_text: str = ""):
    """Create the project-first field workbench used by the normal application entry."""
    from ui.field_workbench_window import FieldWorkbenchWindow

    return FieldWorkbenchWindow(version_text=version_text or "MyGPR 勘探定位工作台")
...
def main():
    ...
    win = create_main_window(version_text=version_text)
