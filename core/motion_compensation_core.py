#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Shared core helpers for UAV motion compensation (used by both atomic nodes and V2)."""
...
def apply_height_correction(
    ...
    *,
    ...
    """Apply height normalization using V2-style core logic."""
    arr = np.asarray(data, dtype=np.float32)
    if arr.ndim != 2:
        raise ValueError("Height correction requires a 2D B-scan array")
    ...
    meta: dict[str, Any] = {
        "method": "height_correction",
        "skipped": False,
        "source_traces": int(trace_count),
        "air_wave_speed_m_per_ns": float(air_wave_speed_m_per_ns),
        "height_reference_mode": str(reference_height_mode),
    ...
    if not metadata:
        ...
        warnings.append(
            _warning("missing_trace_metadata", "trace_metadata is missing; height correction skipped.")
        )
        meta["skipped"] = True
    ...
    if valid_height and air_wave_speed is not None:
        ...
        meta["height_correction_applied"] = bool(
            meta["time_shift_correction_applied"] or meta["amplitude_correction_applied"]
        )
...
def apply_attitude_apc_correction(
    data: np.ndarray,
    trace_metadata: dict[str, Any] | None,
    *,
    ...
    """Apply attitude/APC footprint correction using V2-style core logic."""
    arr = np.asarray(data, dtype=np.float32)
    if arr.ndim != 2:
        raise ValueError("Attitude/APC correction requires a 2D B-scan array")
    ...
    meta: dict[str, Any] = {
        "method": "attitude_apc_correction",
        "skipped": False,
        "source_traces": int(trace_count),
    ...
    if not metadata:
        quality_flags.append("missing_trace_metadata")
        warnings.append(
            _warning("missing_trace_metadata", "trace_metadata is missing; attitude/APC correction skipped.")
        )
        meta["skipped"] = True
        return corrected, meta

    try:
        height_m, _ = _select_height(
            metadata,
            trace_count,
            height_source="auto",
            warnings=warnings,
        ...
        # Keep height_m even if invalid, for projection height fallback
    except ValueError:
        height_m = None
    ...
    if attitude_updates:
        ...
        meta["trace_metadata_updates"] = attitude_updates

    return corrected.astype(np.float32, copy=False), meta


def apply_speed_resampling(
    data: np.ndarray,
    trace_metadata: dict[str, Any] | None,
    *,
    resample_spacing_m: float = 0.0,
) -> Tuple[np.ndarray, dict[str, Any]]:
    """Apply equal-distance resampling using V2-style core logic."""
    from core.trace_metadata_utils import build_uniform_trace_distance_m, resample_trace_metadata

    arr = np.asarray(data, dtype=np.float32)
    if arr.ndim != 2:
        raise ValueError("Speed compensation (resampling) requires a 2D B-scan array")
    ...
    if not metadata:
        quality_flags.append("missing_trace_metadata")
        warnings.append(
            _warning("missing_trace_metadata", "trace_metadata is missing; resampling skipped.")
        )
        meta["skipped"] = True
        return corrected, meta

    if resample_spacing_m and float(resample_spacing_m) > 0.0:
        source_distance = _numeric_field_or_none(metadata, "trace_distance_m", trace_count)
        if source_distance is None:
            # Try to derive from local_x_m/local_y_m
            if "local_x_m" in metadata and "local_y_m" in metadata:
                try:
                    local_x_m = _field_1d(metadata, "local_x_m", trace_count, dtype=np.float64)
                    local_y_m = _field_1d(metadata, "local_y_m", trace_count, dtype=np.float64)
                    source_distance = _compute_trace_distance(local_x_m, local_y_m)
                except ValueError as exc:
                    source_distance = None
                    quality_flags.append("invalid_position_metadata")
                    warnings.append(
                        _warning("invalid_position_metadata", str(exc))
                    )
        ...
        if source_distance is None:
            ...
            meta_for_resampling = _metadata_for_output(metadata, updates, trace_count)
