...
def extract_roi_and_context(
    ...
    t0 = max(0, min(_safe_index(time_start), n_samples - 1))
    t1 = max(t0 + 1, min(_safe_index(time_end, default=n_samples), n_samples))
...
def periodic_banding_ratio(
    ...
    values = tuple(trace_band or ())
    low = max(0.0, _finite_scalar(values[0] if len(values) > 0 else 0.05, 0.05))
    high = min(float(freqs[-1]), _finite_scalar(values[1] if len(values) > 1 else 0.18, 0.18))
    if high <= low:
