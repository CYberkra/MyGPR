...
def _preview_indices(length: int, limit: int) -> np.ndarray:
    ...
    if idx.size == 0 or idx[-1] != length - 1:
        with_tail = np.empty(idx.size + 1, dtype=np.int32)
        with_tail[:-1] = idx
        with_tail[-1] = length - 1
        idx = with_tail
    return idx
...
def build_airborne_georeference_3d_payload(
    ...
    *,
    ...
    payload = {
        ...
        "x_axis_label": "沿测线距离 / 局部 X (m)" if longitude is not None else "沿测线距离 (m)",
        "y_axis_label": "横向偏移 / 局部 Y (m)" if longitude is not None else "横向偏移 (m)",
        "z_axis_label": "高程 / 等效深度 (m)",
