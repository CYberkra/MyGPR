...
- 已调整：`日常处理` 默认直接展示完整参数集，不再需要“显示高级参数”按钮。
...
- V0.8.62 补齐本地环境安装入口、版本一致性检查、Workbench 成果包证据抽屉反馈与 Matplotlib 中文字体兜底。
- V0.8.63 继续收口 Processing Lab 推荐应用门禁与 Interpretation 原始/结果源切换。
- V0.8.64 补齐 Workbench 处理版本资源树的直接打开路径，并增强底部抽屉布局恢复。
...
- `ui/interpretation_workbench_page.py`
- `ui/processing_lab_page.py`
- `app_qt.py`
...
- 尚未在真实 Windows 桌面进行人工点击截图巡检；本轮验证以 offscreen Qt 测试和源码检查为主。
