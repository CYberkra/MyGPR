...
Implementation status: GX-RUN-001/002/003/004 implemented (backend-only)  
...
- `仿真验证` page is implemented as GX-UI-036 for campaign dry-run validation and command preview; direct GUI execution remains intentionally disabled.
- GX-007 scene_001 has a completed CPU small-scale 2D diagnostic artifact (`936 x 21`) generated outside the source repository.
- GPU remains an optional acceleration path; current PowerShell/agent-shell diagnosis indicates the host toolchain can fail when the MSVC compiler environment is not loaded.
...
When the gprMax installation lives in a separate virtual environment, use the
runtime Python explicitly:

```text
python scripts/gprmax_campaign_runner.py --campaign path/to/campaign.yaml --run-scene <scene_id> --variant raw_with_target --num-runs 21 --gprmax-python E:/gprMax/gprMax-v.3.1.7/.venv/Scripts/python.exe
```

Current paired validation and target_response generation command:

```text
python scripts/gprmax_campaign_runner.py --pair-outputs --campaign-id GX-007 --scene-id scene_valid_01 --raw-output path/to/raw.csv --background-output path/to/background.csv --output-dir path/to/output_dir
```

Notes:

- GUI integration now exists for dry-run validation and command preview in `ui/simulation_validation_page.py`; paired preview and direct execution UI are still future work.
- AutoTune evaluation is still not implemented.
- GX-007 has one CPU-generated small 2D diagnostic artifact, but it is not a paper-candidate benchmark and should be archived to Evidence separately.
- Preview/report outputs are diagnostic stubs for human review, not final scientific evidence.
- GPU is an optional acceleration path for NVIDIA CUDA devices only.
- GPU environment checks are lightweight (`nvcc --version`, `pycuda` import) and never make dry-run fail.
- Multi-GPU + MPI scheduling policy is not implemented here; only argument passthrough is supported.
- `run_manifest.json` records GPU request, emitted flags, device ids, and environment probe results.
- GPU acceleration does not change claim boundary semantics.

The next recommended task is:

```text
GX-007-EVIDENCE-002: archive the CPU complete 936 x 21 diagnostic artifact, then optionally GX-RUN-GPU-DIAG-003 in a VS x64 shell
```
