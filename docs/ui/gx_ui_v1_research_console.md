...
The entry point is the existing `调参与实验` tab. A new segmented page named `研究验证` hosts four read-only pages:
...
The main window stack, right-side B-scan canvas, and legacy Workbench fallback remain unchanged.
