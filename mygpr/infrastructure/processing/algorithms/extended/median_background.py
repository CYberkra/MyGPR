...
def method_median_background_2d(
    ...
    **kwargs,
    ...
    selection = resolve_time_selection(
        arr.shape,
        {
    ...
    result = apply_time_selection(
        arr, full_result, selection, int(edge_taper_samples)
    )
    return result.astype(np.float32, copy=False), {
        "method": "median_background_2D",
        "ntraces": ntraces,
        "time_start_idx": int(selection.start),
        "time_end_idx": int(selection.end),
