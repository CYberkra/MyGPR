...
def test_v0912_layout_metrics_fit_windows_125_percent_work_area() -> None:
    ...
    assert 360 <= metrics.processing_bscan_h <= 430
    ...
    assert metrics.spatial_map_h >= 330
    assert metrics.spatial_map_h > metrics.spatial_dem_h * 4
    assert metrics.delivery_side_min_w <= 300
    assert metrics.delivery_files_max_h <= 135
...
def test_v0912_layout_metrics_scale_up_without_exceeding_bounds() -> None:
    ...
    assert metrics.processing_bscan_h >= 500 or metrics.processing_bscan_h == 500
    ...
    assert metrics.spatial_map_h <= 500
