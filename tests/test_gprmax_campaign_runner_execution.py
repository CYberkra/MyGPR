    ...
    campaign_path.write_text(
        "\n".join(
            [
                ...
                f"    raw_model: {str((FIXTURE_DIR / 'fake_gprmax_success.py').resolve().as_posix())}",
                f"    background_model: {str((FIXTURE_DIR / 'fake_gprmax_fail.py').resolve().as_posix())}",
                f"    materials: {str((FIXTURE_DIR / 'models' / 'materials.txt').resolve().as_posix())}",
                f"    target_roi: {str((FIXTURE_DIR / 'annotations' / 'target_roi.json').resolve().as_posix())}",
                "    expected_outputs:",
                "      - raw_with_target",
                "      - background_only",
                "      - target_response",
