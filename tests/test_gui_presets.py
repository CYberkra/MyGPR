...
def test_auto_tune_defaults_live_in_auto_tune_page():
    ...
    try:
        ...
        assert win.page_auto_tune.btn_compare_manual_auto.text() == "手动/推荐对比"
        assert win.page_auto_tune.btn_export_comparison.isEnabled() is False
        win.page_auto_tune.segmented.setCurrentItem("actions")
        assert win.page_auto_tune.stack.currentWidget() is win.page_auto_tune.page_actions
        assert win.page_advanced.roi_status_label.text() == "手动 关注范围: 未设置"
        assert win.page_advanced.btn_clear_manual_roi.isEnabled() is False
        assert win.page_basic.action_apply_manual.text() == "使用当前参数（默认）"
        assert win.page_basic.action_apply_auto_tuned.text() == "使用推荐参数"
...
def test_auto_tune_page_handles_state_transitions():
    ...
    try:
        ...
        assert "支持参数推荐" in win.page_auto_tune.auto_tune_summary.toPlainText()
        ...
        assert "关注范围来源: 当前裁剪区" in running_text
        ...
        assert "参数推荐失败:" in win.page_auto_tune.auto_tune_summary.toPlainText()
        ...
        assert "正在生成手动/推荐对比" in win.page_auto_tune.comparison_summary.toPlainText()
        assert win.page_auto_tune.btn_export_comparison.isEnabled() is False
...
def test_auto_tune_truth_validation_page_displays_ground_truth_metadata():
    ...
    try:
        ...
        assert win.page_auto_tune.truth_status_label.text() == "参考标注已启用。"
...
def test_auto_tune_truth_validation_incomplete_ground_truth_info_is_warning():
    ...
    try:
        ...
        assert "缺少完整的目标/背景关注范围" in win.page_auto_tune.truth_status_label.text()
...
def test_phase2_tabs_expose_prioritized_group_hierarchy_and_bridge():
    ...
    try:
        ...
        assert win.page_auto_tune.trial_table.columnCount() == 8
        assert win.page_auto_tune.btn_open_workbench.text() == "进入工作台测线处理"

        quality_group_titles = []
        for index in range(win.page_quality.quality_section_stack.count()):
            quality_group_titles.extend(
                _top_level_group_titles(win.page_quality.quality_section_stack.widget(index))
            )
        assert quality_group_titles == [
            "质量概览",
            "质量图表",
            "处理记录",
            "报告与导出",
...
def test_main_plot_uses_index_axes_until_physical_axes_enabled():
    app = _get_app()
    win = GPRGuiQt()
    try:
        ...
        trace_axis = win._build_trace_axis(3)
        assert np.array_equal(sample_axis, np.arange(5, dtype=np.float32))
        assert np.array_equal(trace_axis, np.arange(3, dtype=np.float32))
        ...
        assert cfg["xlabel"] == "道数"
        assert cfg["ylabel"] == "采样点"

        win.page_advanced.show_physical_x_axis_var.setChecked(True)
        win.page_advanced.show_physical_y_axis_var.setChecked(True)
        time_axis = win._build_time_axis(5)
        distance_axis = win._build_trace_axis(3)
        assert np.allclose(time_axis, np.linspace(0.0, 120.0, 5, dtype=np.float32))
        assert np.allclose(distance_axis, np.array([0.0, 0.5, 1.0], dtype=np.float32))
        cfg = win._resolve_plot_extent_and_labels(
            np.zeros((5, 3), dtype=np.float32),
            None,
            {
                "time_axis": time_axis,
                "trace_axis": distance_axis,
            },
        )
        assert cfg["xlabel"] == "距离 (m)"
...
def test_auto_tune_tab_hides_research_console_pages_in_field_mode():
    app = _get_app()
    win = GPRGuiQt()
    try:
        assert win.page_auto_tune.page_research is None
        labels = [item.text() for item in win.page_auto_tune.segmented.items.values()]
        assert "开发工具" not in labels
        assert "参考标注" not in labels
    finally:
        win.close()
        app.processEvents()
...
def test_manual_roi_is_prioritized_for_auto_tune_when_present():
    ...
    try:
        ...
        assert roi_spec["label"] == "手动框选 关注范围"
    finally:
        win.close()
        app.processEvents()
...
def test_apply_menu_switches_default_source_mode():
    ...
    try:
        ...
        assert (
            win.page_basic.action_apply_auto_tuned.text() == "使用推荐参数（默认）"
        )
...
def test_start_auto_select_current_stage_uses_same_stage_public_methods(monkeypatch):
    ...
    try:
        ...
        import ui.main_window_autotune_start_mixin as autotune_start_module

        monkeypatch.setattr(autotune_start_module, "QThread", DummyThread)
        monkeypatch.setattr(autotune_start_module, "AutoTuneStageWorker", DummyWorker)
    ...
    try:
        ...
        payload = build_airborne_georeference_3d_payload(
            ...
            {"total_time_ns": 80.0},
            {
                ...
                "longitude": np.linspace(104.0, 104.0001, 8),
                "latitude": np.linspace(30.0, 30.0001, 8),
            },
        )
...
def test_georef3d_pyvista_grid_preserves_curtain_scalars():
    app = _get_app()
    page = QualityLogPage()
    try:
        ...
        grid = page._build_georef3d_pyvista_grid(
            x,
            y,
            z,
            amplitude,
        ...
        assert "amplitude" in grid.point_data
        assert grid.point_data["amplitude"].shape == (12,)
        assert np.isclose(grid.point_data["amplitude"][0], amplitude.reshape(-1, order="F")[0])
    finally:
        page.release_plot_resources()
        page.close()
        app.processEvents()
