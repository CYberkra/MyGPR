...
def test_processed_artifacts_use_timestamped_params_and_exact_index_mapping(tmp_path: Path) -> None:
    ...
    for path in manifest_files:
        ...
        assert payload["save_schema"] == "mygpr.processing_save.v2"
...
def test_processing_page_save_does_not_use_synthetic_fallback() -> None:
    source = Path("ui/field_panels/processing_page.py").read_text(encoding="utf-8")
    start = source.index("def _save_processing_result")
    end = source.index("def _recommend_processing_params", start)
    body = source[start:end]
    assert "synthetic_bscan" not in body
    assert "当前处理结果与所选测线不一致" in body
    # v0.9.24 updated the error message wording
    assert "当前处理结果与所选测线不一致" in body
