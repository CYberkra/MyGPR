...
class AutoTuneResultDialog(QDialog):
    ...
    def _build_summary_text(self) -> str:
        ...
        return (
            f"方法: {method_name} | 类型: {family} | 关注范围: {roi_label} | 候选数: {trials} | 失败候选: {failed_trials} | 缓存命中: {cache_hits} | 推荐档: {recommended} | 稳定性: {confidence:.2f} | 风险: {risk_flags} | 建议: {recommendation} | 最优得分: {best_score:.4f} | "
    ...
    def _build_reason_text(self) -> str:
        ...
        if roi_info:
            lines.append("")
            lines.append(
                f"关注范围来源: {roi_info.get('label') or roi_info.get('source') or '全图'}"
            )
            lines.append(
