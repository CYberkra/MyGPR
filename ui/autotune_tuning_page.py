...
- 专门服务 参数推荐，不引入 研究实验室 / 三维查看器。
- 针对 MyGPR 当前右侧工作区宽度重排为“顶部状态 + 标签页”结构。
- 仅做 UI 状态联动，不触发 参数推荐/开发验证/交付记录 执行。
- 保留 旧版 AutoTunePage 兼容层，避免 app_qt 既有信号/状态调用断裂。
"""
...
class AutoTuneTuningPage(QWidget):
    ...
    _CANDIDATES = [
        ...
        ("sliding", "滑动窗口背景", "局部窗口背景估计；适合变化较大的背景。"),
    ]

    _SCORING = [
        ("roi_retention", "关注范围能量保持", "关注范围内响应保留程度。"),
        ("residual", "背景残差降低", "关注范围外背景/杂波残差下降程度。"),
        ("cnr", "CNR/SNR 提升", "目标与背景可分性变化。"),
        ("shape", "目标形态稳定性", "双曲线/目标区域连续性与位移变化。"),
        ("rmse", "参考响应误差", "仅带参考响应的数据可用于量化。"),
    ]
    ...
    def set_loaded_dataset(
        ...
        *,
        ...
        does not mutate production scoring, and does not write processing records.
        """
    ...
    def _build_ui(self) -> None:
        ...
        self.advanced_toggle.setText("高级设置与运行明细 ▸")
        self.advanced_toggle.setCheckable(True)
        self.advanced_toggle.setToolTip("展开关注范围、候选参数、评分说明、报告和运行日志。")
        ...
        # 旧版 AutoTunePage 仍保留为信号兼容层。主窗口既有 start_auto_tune_* 连接仍指向
    ...
    def _build_top_control_bar(self) -> QFrame:
        ...
        self.btn_auto_roi.setToolTip("根据当前 B-scan 响应建议关注范围；这里只是处理范围，不代表最终目标结论。")
        ...
        self.btn_generate_recommendation.setToolTip("按当前处理目标、关注范围和候选参数生成推荐流程。")
    ...
    def _build_recipe_detail_tabs(self) -> QTabWidget:
        ...
        self.detail_tabs.addTab(self._build_candidate_compare_tab(), "候选参数")
    ...
    def _build_roi_detail_tab(self) -> QWidget:
        ...
        layout.setContentsMargins(8, 8, 8, 8)
        layout.setSpacing(8)
        self.roi_mode_hint = QLabel("关注范围用于限定处理建议；不确定时可保持全图。")
        ...
        for row, (label, widget) in enumerate(
            [
                ("起始道号", self.roi_trace_start),
                ("结束道号", self.roi_trace_end),
                ("起始采样点", self.roi_sample_start),
                ("结束采样点", self.roi_sample_end),
            ]
        ):
            coord_layout.addWidget(QLabel(label), row, 0)
            coord_layout.addWidget(widget, row, 1)
        layout.addWidget(coord_box)
        self.roi_picker_status_label = QLabel("关注范围：全图")
    ...
    def _toggle_advanced_panel(self, checked: bool) -> None:
        ...
        if toggle is not None:
            toggle.setText("高级设置与运行明细 ▾" if checked else "高级设置与运行明细 ▸")
        if checked:
    ...
    def _build_header(self) -> QFrame:
        ...
        subtitle = QLabel("选择处理目标和关注范围后，软件生成推荐处理流程与参数。")
        ...
        self.btn_load_data.setToolTip("请在左侧主页面导入数据；本页会自动同步当前数据状态。")
        self.btn_run_autotune_mvp.setToolTip("在当前数据、关注范围和候选参数下生成推荐流程。")
        self.btn_export_evidence_mvp.setToolTip("导出自动推荐报告；详细候选记录在高级设置与运行明细中查看。")
        ...
        self.btn_open_research_console = self._action_button("开发工具", primary=False)
        self.btn_open_research_console.setToolTip(
            "开发模式入口：查看只读验证与技术记录。"
        )
        self.btn_open_research_console.clicked.connect(self._open_research_console)
        if is_research_ui_enabled():
            actions.addWidget(self.btn_open_research_console)
        else:
            self.btn_open_research_console.hide()
    ...
    def _build_target_region_tab(self) -> QWidget:
        """Wizard step 2: focus area and review boundaries."""
        page = QWidget()
        layout = QVBoxLayout(page)
        layout.setContentsMargins(8, 8, 8, 8)
        layout.setSpacing(12)

        intro = QGroupBox("关注范围")
        intro_layout = QVBoxLayout(intro)
        intro_layout.setContentsMargins(10, 12, 10, 10)
        intro_layout.setSpacing(8)
        hint = QLabel("默认使用全图；也可以选择自动建议或手动框选。")
        hint.setObjectName("Hint")
    ...
    def _build_advanced_settings_tab(self) -> QTabWidget:
        ...
        tabs.addTab(self._build_audit_tab(), "报告 / 技术记录")
        self.research_console_panel = None
        self.research_console_tab_index = None
        if is_research_ui_enabled():
            self.research_console_panel = ResearchConsolePage(self)
            self.research_console_tab_index = tabs.addTab(self.research_console_panel, "开发验证")
        return tabs

    def _open_research_console(self) -> None:
        """Open the read-only research console from the primary AutoTune page."""
        if not is_research_ui_enabled():
            return
        if getattr(self, "advanced_toggle", None) is not None:
            self.advanced_toggle.setChecked(True)
        panel = getattr(self, "advanced_panel", None)
        index = getattr(self, "research_console_tab_index", None)
        if panel is not None and isinstance(index, int):
            panel.setCurrentIndex(index)
        research = getattr(self, "research_console_panel", None)
        if research is not None and hasattr(research, "refresh"):
            research.refresh()
    ...
    def _build_roi_box(self) -> QGroupBox:
        ...
        self.roi_mode_hint = QLabel("默认使用全图，适合快速推荐。")
        self.roi_mode_hint.setObjectName("Hint")
        self.roi_mode_hint.setWordWrap(True)
        ...
        self.btn_pick_roi.setCheckable(True)
        self.btn_pick_roi.setChecked(False)
        self.btn_pick_roi.setToolTip("选择“手动”后，在右侧 B-scan 上拖拽框选关注范围。")
        self.btn_pick_roi.toggled.connect(self._on_pick_roi_toggled)
        self.btn_auto_roi = QPushButton("自动")
        self.btn_auto_roi.setEnabled(False)
        self.btn_auto_roi.clicked.connect(self._on_auto_roi_clicked)
    ...
    def set_plot_roi_picker_status(self, enabled: bool) -> None:
        """由主窗口同步图上关注范围框选开关状态。"""
        text = "关注范围：框选已开启" if enabled else self._roi_mode_label()
    ...
    def _on_auto_roi_clicked(self) -> None:
        ...
        self.state.backend_message = "已基于当前 B-scan 响应强度生成自动建议区域；该区域不是人工确认标注"
        self._sync_controls_from_state()
        self._mark_recommendation_dirty()
        self._refresh_from_state()
    ...
    def _update_roi_mode_controls(self) -> None:
        ...
        if getattr(self, "roi_mode_hint", None) is not None:
            if mode == "manual":
                text = "手动范围：在 B-scan 上框选重点区域。"
            elif mode == "auto":
                text = "自动范围：按当前响应估计关注范围，仅作为处理参考。"
            else:
                text = "关注范围：全图。"
    ...
    def _refresh_from_state(self) -> None:
        ...
        roi_text = {"none": "关注范围：全图", "auto": "关注范围：自动", "manual": "关注范围：手动"}.get(self.state.roi_mode, "关注范围：全图")
    ...
    def _refresh_preview_cards(self) -> None:
        ...
        if hasattr(self, "candidate_preview_canvas"):
            self.candidate_preview_canvas.setText(
                f"当前候选输出\n\n候选数：{self.state.candidate_count}\n关注范围：道号 {self.state.roi_trace_start}-{self.state.roi_trace_end} / 采样点 {self.state.roi_sample_start}-{self.state.roi_sample_end}" if loaded else "当前候选输出\n\n等待数据载入"
            )
    ...
    def _refresh_apply_report_text(self) -> None:
        ...
        if self.state.data_source != "已载入":
            text.setText("尚未载入数据。请先在主页面导入测线，然后返回参数推荐。")
            return
        scoring_record = self._current_scoring_record()
        lines = [
    ...
    def _refresh_audit_text(self) -> None:
        self.metrics_text.setText(
            "\n".join(
                [
                    ...
                    f"关注范围已设置 = {self.state.roi_is_set}",
                    ...
                    "说明：当前使用受控的流程模板和候选参数生成推荐，不做无边界自由搜索。",
                ]
            )
        )
        ...
        self.warnings_text.setText(
            "\n".join(
                [
                    "推荐方案来自当前候选参数范围，不代表所有场景的唯一最优方案。",
                    "真实数据没有参考响应时，排序依据为启发式指标。",
                    "报告中会记录目标倾向、关注范围、候选参数和参数来源。",
                ]
            )
        )
    ...
    def _claim_boundary_text(self) -> str:
        return (
            f"该推荐表示在当前处理目标“{self.state.target_goal}”、候选空间、关注范围模式和评分指标设置下，"
            "系统生成的一套处理流程与参数建议。"
    ...
    def _roi_mode_label(self) -> str:
        if self.state.roi_mode == "manual":
            if self.state.roi_is_set:
                return (
                    f"手动关注范围 T{self.state.roi_trace_start}-{self.state.roi_trace_end} / "
