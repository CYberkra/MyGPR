...
manual focus-area drag, slider compare dragging, coordinate readout, and view-limit
...
class BscanInteractionController:
    ...
    def on_main_canvas_release(self, event):
        """Handle click selection, focus-area commit, slider commit, and pan completion."""
        host = self.host
    ...
    def on_main_canvas_key_press(self, event):
        """Esc clears transient states; R toggles focus area; Home/H resets view."""
        host = self.host
        key = str(getattr(event, "key", "") or "").lower()
        if key == "escape":
            host._main_press_state = None
    ...
    def draw_drag_roi_preview(self, start: dict, event):
        """Draw throttled transient focus-area rectangle while dragging."""
        host = self.host
        ax = start.get("axes")
    ...
    def remove_drag_roi_preview(self):
        """Remove transient focus-area preview rectangle."""
        host = self.host
    ...
    def draw_manual_roi_marker(self, axes, axis_info: dict):
        """Draw current manual focus area on the main B-scan."""
        host = self.host
