...
from core.product_mode import is_research_ui_enabled
...
class AutoTunePage(QWidget):
    ...
    def setup_ui(self):
        ...
        title = QLabel("自动推荐与参数对比")
        title.setProperty("class", "sectionTitle")
        layout.addWidget(title)

        page_hint = QLabel(
            "本页集中处理候选参数分析、同阶段方法比较与推荐结果复核。分析完成后，可回到“日常处理”页把“应用方法”的默认来源切换为“使用推荐参数”。"
        )
        ...
        flow_hint = QLabel(
            "推荐顺序：先配置关注范围与搜索强度，再生成推荐或做同阶段比较，最后查看结果并决定是否采用推荐方案。"
        )
        ...
        self.segmented.addItem("actions", "推荐生成")
        self.segmented.addItem("results", "结果查看")
        if is_research_ui_enabled():
            self.segmented.addItem("truth", "参考标注")
            self.segmented.addItem("research", "开发工具")
        ...
        self.page_research = ResearchConsolePage(self) if is_research_ui_enabled() else None
        ...
        if is_research_ui_enabled():
            ...
            if self.page_research is not None:
    ...
    def _build_config_page(self) -> QWidget:
        ...
        method_box = QGroupBox("当前方法与推荐边界")
        method_box.setToolTip("先确认当前方法是否支持参数推荐，再决定是在本页生成推荐还是进入测线处理")
        ...
        method_hint = QLabel(
            "本页适合做单方法参数推荐和同阶段快速比较。需要跨方法串联、手工复核或保存完整处理链路时，再进入测线处理。"
        )
        ...
        config_hint = QLabel(
            "关注范围决定评分聚焦区域，搜索决定候选数量与细化强度。一般先用“当前裁剪区优先 + 标准”。"
        )
        ...
        form_layout.addWidget(QLabel("关注范围来源"), 0, 0)
        self.auto_tune_roi_combo = QComboBox()
        self.auto_tune_roi_combo.addItem("当前裁剪区优先", "prefer_crop")
        self.auto_tune_roi_combo.addItem("自动建议范围", "auto")
        self.auto_tune_roi_combo.addItem("全图", "full")
        self.auto_tune_roi_combo.setToolTip("生成推荐时优先使用哪一块区域")
    ...
    def _build_actions_page(self) -> QWidget:
        ...
        action_box = QGroupBox("自动推荐操作")
        action_box.setToolTip("对当前方法进行自动量化评估、候选搜索、同阶段比较与推荐结果查看")
        action_layout = QVBoxLayout(action_box)
        action_layout.setContentsMargins(10, 14, 10, 10)
        action_layout.setSpacing(10)

        auto_hint = QLabel(
            "建议先完成候选分析或同阶段比较，再决定是否把“应用方法”的默认来源切换为推荐参数。"
        )
        ...
        self.btn_auto_tune = PushButton(FluentIcon.SETTING, "生成推荐")
        self.btn_auto_tune.setToolTip("对当前方法的候选参数评分并生成推荐参数")
        self.btn_compare_stage = PushButton(FluentIcon.FILTER, "同阶段方法比较")
        self.btn_compare_stage.setToolTip("比较当前 stage 内多个可用方法，推荐更合适的方法")
        self.btn_compare_stage.setEnabled(False)
        self.btn_compare_manual_auto = PushButton(FluentIcon.VIEW, "手动/推荐对比")
        self.btn_compare_manual_auto.setToolTip("用经验/当前参数 baseline 与参数推荐结果生成对比")
        self.btn_view_auto_tune = PushButton(FluentIcon.VIEW, "查看推荐结果")
        ...
        self.btn_open_workbench = PushButton(FluentIcon.APPLICATION, "进入工作台测线处理")
        self.btn_open_workbench.setToolTip("需要跨方法组合、手工调参与长链路处理时，跳转到工作台继续")
    ...
    def _build_results_page(self) -> QWidget:
        ...
        detail_hint = QLabel("把参数推荐与同阶段比较拆开查看，减少文本挤压并提升判读速度。")
        ...
        self.result_segmented.addItem("auto", "参数推荐结果")
        self.result_segmented.addItem("stage", "阶段比较结果")
        self.result_segmented.addItem("comparison", "手动/推荐对比")
        ...
        self.auto_tune_summary.setPlaceholderText("参数推荐结果将在这里显示：推荐参数、候选评分摘要与三档候选。")
        ...
        comparison_panel_hint = QLabel("显示人工 baseline 与参数推荐的参数、评分差异和图像对比入口。")
        ...
        self.comparison_summary.setPlaceholderText("手动/推荐对比结果会显示在这里：pipeline、参数差异、评分差异和结论。")
        ...
        self.btn_export_comparison = PushButton(FluentIcon.SAVE, "导出对比报告")
        self.btn_export_comparison.setEnabled(False)
        self.btn_export_comparison.setToolTip(
            "导出摘要 JSON、手动/推荐对比图、参数表和指标表"
        )
    ...
    def _build_truth_validation_page(self) -> QWidget:
        ...
        status_box = QGroupBox("参考标注状态")
        status_layout = QVBoxLayout(status_box)
        status_layout.setContentsMargins(10, 14, 10, 10)
        status_layout.setSpacing(10)

        self.truth_status_label = QLabel("当前数据未检测到参考标注文件。普通参数推荐可正常使用。")
        ...
        rows = [
            ...
            ("target 关注范围", "truth_target_roi_label"),
            ("background 关注范围", "truth_background_roi_label"),
        ...
        self.truth_bscan_status_label = QLabel("尚未运行手动/推荐对比。运行后可查看原始数据、手动参数和推荐参数的对比结果，并导出对比报告。")
        ...
        self.truth_side_by_side_preview = QLabel("暂无对比预览。导出对比报告后会显示缩略图。")
        ...
        self.btn_truth_open_side_by_side = PushButton(FluentIcon.VIEW, "打开对比图片")
        ...
        self.truth_params_table.setHorizontalHeaderLabels(["方法", "手动参数", "推荐参数"])
        ...
        self.btn_truth_run_comparison = PushButton(FluentIcon.VIEW, "运行手动/推荐对比")
        self.btn_truth_export_evidence = PushButton(FluentIcon.SAVE, "导出对比报告")
        ...
        self.truth_evidence_label = QLabel("对比报告尚未导出。")
    ...
    def _on_segment_changed(self, route_key: str):
        if route_key == "truth" and is_research_ui_enabled():
            self.refresh_truth_validation()
        if route_key == "research" and is_research_ui_enabled() and hasattr(self, "page_research") and self.page_research is not None:
            self.page_research.refresh()
        mapping = {"config": 0, "actions": 1, "results": 2}
        if is_research_ui_enabled():
            mapping.update({"truth": 3, "research": 4})
    ...
    def refresh_truth_validation(self):
        ...
        if ground_truth:
            self.truth_status_label.setText("参考标注已启用。")
            ...
            self.truth_status_label.setText(
                "参考标注存在，但缺少完整的目标/背景关注范围；请加载参考数据清单和目标范围文件。"
            )
        ...
        if not comparison:
            if side_by_side:
                self.truth_bscan_status_label.setText(
                    f"已记录 对比报告 预览路径: {side_by_side}"
                )
            else:
                self.truth_bscan_status_label.setText(
                    "尚未运行手动/推荐对比。运行后可查看原始数据、手动参数和推荐参数的对比结果，并导出对比报告。"
                )
            return
        if side_by_side:
            self.truth_bscan_status_label.setText(
                f"原始数据 / 手动参数 / 推荐参数对比已生成；对比文件：{side_by_side}"
            )
        else:
            self.truth_bscan_status_label.setText(
                "已生成原始数据、手动参数和推荐参数的对比结果，可导出对比报告查看图像和参数表。"
            )
    ...
    def _refresh_truth_side_by_side_preview(self, side_by_side_path):
        ...
        if not side_by_side_path:
            self._truth_side_by_side_preview_cache_key = None
            self._truth_side_by_side_preview_cache_pixmap = QPixmap()
            self.truth_side_by_side_preview.setText("暂无对比预览。导出对比报告后会显示缩略图。")
            return
        ...
        if not exists:
            self.truth_side_by_side_preview.setText(f"对比图片路径已记录，但文件不存在：\n{path_text}")
            return
        ...
        if pixmap.isNull():
            self._truth_side_by_side_preview_cache_key = None
            self._truth_side_by_side_preview_cache_pixmap = QPixmap()
            self.truth_side_by_side_preview.setText(f"对比图片路径已记录，但无法载入缩略图：\n{path_text}")
            return
    ...
    def _refresh_truth_evidence_controls(self):
        ...
        if output_dir or report_path:
            ...
            self.truth_evidence_label.setText("对比报告 尚未导出。")

    def set_evidence_export_result(self, bundle: dict | None):
        """记录最近一次 对比报告 导出结果并刷新参考标注入口。"""
    ...
    def show_comparison_running(self, roi_label: str, search_mode: str):
        ...
        self._set_result_overview(
            state="对比中",
            stats=f"关注范围={roi_label} | 搜索={search_mode}",
            risk="正在以同一输入、同一 关注范围 运行人工 baseline 与参数推荐分支。",
        )
    ...
    def show_comparison_error(self, error_msg: str):
        ...
        self.result_segmented.setCurrentItem("comparison")
        self.comparison_summary.setPlainText(f"手动/推荐对比失败:\n{error_msg}")
    ...
    def _format_comparison_summary(self, summary: dict) -> str:
        ...
        lines.append(f"关注范围: {roi_info.get('label', roi_info.get('source', '--'))}")
    ...
    def show_cancelled(self):
        ...
        self.set_auto_tune_summary("参数推荐已取消。")

    def show_error(self, error_msg: str):
        """显示分析失败状态。"""
        self.set_auto_tune_result_available(False)
        self._set_result_overview(state="失败", risk="当前没有可用推荐结果。")
        self.set_auto_tune_summary(f"参数推荐失败:\n{error_msg}")
    ...
    def _build_risk_hint(self, result: dict) -> str:
        ...
        if risk_flags:
            ...
            if risk_reason:
                return f"参数推荐存在风险标记: {detail}。{risk_reason}"
            return f"参数推荐存在风险标记: {detail}。建议结合参数域与候选明细复核。"
        if failed > 0:
    ...
    def _format_result_summary(self, result: dict) -> str:
        ...
        lines.append(f"关注范围来源: {roi_label}")
        lines.append(
