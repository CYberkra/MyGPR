...
def _preferred_cjk_font_candidates() -> list[str]:
    from ui.matplotlib_fonts import preferred_cjk_font_candidates

    return preferred_cjk_font_candidates()


def _register_qt_cjk_fonts() -> set[str]:
    """Register common CJK font files when the Qt platform exposes no fonts."""
    windows_root = os.environ.get("WINDIR") or os.environ.get("SystemRoot") or ""
    windows_fonts = os.path.join(windows_root, "Fonts")
    font_paths = [
    ...
    for path in font_paths:
        ...
        try:
            ...
            if font_id >= 0:
                registered.update(QFontDatabase.applicationFontFamilies(font_id))
        except Exception:
            continue
    return registered
...
def _register_matplotlib_cjk_fonts() -> list[str]:
    ...
    return list(configure_matplotlib_cjk_fonts())
...
def _configure_qt_cjk_font(app: QApplication) -> str:
    ...
    try:
        ...
        families = set()
    if not any(family in families for family in candidates):
        families.update(_register_qt_cjk_fonts())
...
def build_version_string(app_name: str = "MyGPR") -> str:
    ...
    shortsha = _get_git_short_sha(os.path.dirname(BASE_DIR))
    if release:
        return f"{app_name} v{release} ({shortsha})"
