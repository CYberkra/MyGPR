...
class MainWindowAutoTuneStartMixin:
    def start_auto_tune_current_method(self, auto_apply_after_finish: bool = False):
        ...
        if (
            ...
            QMessageBox.information(self, "参数推荐", "当前已有任务在运行，请稍候。")
            return False
        if self.data is None or self.data_path is None:
            self._pending_apply_after_auto_tune = False
            QMessageBox.warning(self, "参数推荐", "请先导入数据。")
            return False
        ...
        if not method_key or not method_info.get("auto_tune_enabled"):
            self._pending_apply_after_auto_tune = False
            QMessageBox.information(self, "参数推荐", "当前方法暂不支持自动推荐参数。")
            return False
        ...
        try:
            ...
            self._pending_apply_after_auto_tune = False
            QMessageBox.warning(self, "参数推荐", str(e))
            return False
        ...
        self._set_busy(True, text=f"参数推荐: {method_info.get('name', method_key)}")
