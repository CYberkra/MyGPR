...
class MainWindowExportMixin:
    ...
    def export_auto_tune_comparison_artifacts(self):
        ...
        def _export():
            with self._perf().span("export.autotune_comparison_bundle_ms"):
                return export_auto_tune_comparison_bundle(
                    ...
                    input_ref=input_ref,
                    notes=[
                        "同一输入、同一关注范围、同一锁定色标下导出的手动参数与自动推荐参数对比报告。",
                        "仿真数据和真实外业数据可复用该导出格式做后续复核。",
                    ],
                    cmap=cmap,
                )
        ...
        def _done(bundle):
            ...
            self._log("对比报告已导出: " + "; ".join(display_paths))
            self.status_label.setText("手动/推荐对比报告导出完成")
            QMessageBox.information(
                self,
                "导出成功",
        ...
        def _failed(traceback_text):
            ...
            self.status_label.setText("手动/推荐对比报告导出失败")
            QMessageBox.critical(self, "导出失败", f"对比报告导出失败：\n{traceback_text}")
    ...
    def export_replay_evidence_bundle(self):
        ...
        if not package:
            QMessageBox.information(
                self, "无可导出内容", "请先导入数据并至少完成一次处理或查看。"
            )
            return
        ...
        def _done(result):
            ...
            try:
                ...
                zip_disp = zip_path
            self._log(f"处理历史回放文件已导出: {zip_disp}")
            self.status_label.setText("处理历史回放文件导出完成")
            QMessageBox.information(self, "导出成功", f"已导出:\n{zip_path}")

        def _failed(traceback_text):
            logger.error("Failed to export replay evidence bundle: %s", traceback_text)
            self.status_label.setText("处理历史回放文件导出失败")
            QMessageBox.critical(self, "导出失败", f"处理记录导出失败：\n{traceback_text}")
