# -*- coding: utf-8 -*-
"""Project report export controller for MyGPR.
...
forwarded to the host window, while report-specific builders and attached-file writers
...
class ReportExportController:
    """Generate MyGPR project report packages for a host main window.
    ...
    def generate_report(self):
            ...
            package_dir = os.path.join(out_dir, f"MyGPR_Project_Report_{ts}")
            os.makedirs(package_dir, exist_ok=True)

            # Canonical report-package paths.  These stable file names make the
            ...
            lines = []
            lines.append(f"# MyGPR 项目报告 ({ts})")
            lines.append("")
            lines.append("> 本报告记录当前测线处理图像、处理参数、检查提示和适用边界，便于项目复核与交付。")
            lines.append("")
            ...
            lines.append("```")
            if last_run:
                lines.append(f"- 最近运行: {last_run.get('label', method_name)}")
                lines.append(f"- 运行类型: {last_run.get('run_type', 'unknown')}")
                lines.append(f"- 运行时间: {last_run.get('timestamp', '--')}")
                if last_run.get("preset_key"):
                    lines.append(f"- 推荐档位: {last_run['preset_key']}")
                if last_run.get("profile_key"):
                    lines.append(f"- 参数档位: {last_run['profile_key']}")
            ...
            lines.append(f"- 当前方法: {method_name}")
            ...
            lines.append(
                f"- 对称拉伸: {self.page_advanced.symmetric_var.isChecked()}"
            )
            ...
            lines.append(f"- 归一化: {self.page_advanced.normalize_var.isChecked()}")
            lines.append(f"- 去均值: {self.page_advanced.demean_var.isChecked()}")
            if bounds:
                lines.append(
                    f"- 裁剪: time {bounds['time_start']}~{bounds['time_end']} ; distance {bounds['dist_start']}~{bounds['dist_end']}"
                )
            else:
                lines.append("- 裁剪: disabled")
            quality = self._last_quality_metrics or {}
            if quality:
                lines.append(
                    "- 质量指标: focus_ratio={focus_ratio:.4f}, hot_pixels={hot_pixels}, spikiness={spikiness:.3f}, time_ms={time_ms:.1f}".format(
                        **quality
                    )
                )
            else:
                lines.append("- 质量指标: --")
            lines.append("")
            lines.append(f"- 报告目录: {package_dir}")
            lines.append(f"- Screenshot: {image_path}")
            lines.append(f"- HTML preview: {html_path}")
            lines.append(f"- 项目清单: {manifest_path}")
            lines.append(f"- 文件索引: {evidence_index_path}")
            lines.append(f"- 处理流程文件: {workflow_path}")
            lines.append(f"- 处理链文件: {processing_chain_path}")
            lines.append(f"- 运行日志: {runtime_log_path}")
            lines.append(f"- 运行事件: {runtime_events_path}")
            lines.append(f"- 参数文件: {params_path}")
            lines.append(f"- 显示设置: {display_settings_path}")
            lines.append(f"- 关注范围文件: {roi_path}")
            lines.append(f"- 图像清单: {figure_manifest_path}")
            lines.append(f"- 适用边界: {claim_boundary_path}")
            lines.append(f"- 输入数据识别: {input_identity_path}")
            lines.append(f"- 检查提示文件: {warnings_path}")
            lines.append(f"- 复核说明: {audit_note_path}")
            lines.append("")
            lines.append("## 运行状态")
            lines.append(
            ...
            if scoring_record:
                try:
                    ...
                    lines.append("- 参数推荐评分: 已记录在附属文件中")
            if self._no_prior_guard_events:
                lines.append("- 安全策略触发记录:")
            ...
            try:
                ...
                logger.exception("Failed to write report attached files")
                self._log(f"报告附属文件生成失败: {e}")
    ...
    def _build_processing_chain_export(self) -> list[dict]:
            ...
            Report attached files describe the same Raw/current/warning/pruned states.
    ...
    def _build_report_input_identity(self) -> dict:
            """Build a conservative input identity block for report attached files."""
    ...
    def _build_report_software_version(self, timestamp: str) -> dict:
            """Build software version metadata for an Report package."""
    ...
    def _build_report_roi_payload(self, timestamp: str) -> dict:
            """Record current 关注范围 state separately from processing and display settings."""
            ...
            return {
                ...
                "roi_claim": "关注范围 records UI selection state only; it is not 参考标注 unless explicitly provided by a validated dataset contract.",
            }
    ...
    def _build_report_audit_note(
            ...
            *,
            ...
            lines = [
                ...
                "## 复核提醒",
                "",
                "本报告记录当前数据处理状态和参数。目标判断仍需要结合现场资料、专业复核和项目验收要求。",
            ]
            return "\n".join(lines) + "\n"
    ...
    def _write_report_sidecars(
            ...
            *,
            ...
            """Write structured attached files for the field report package."""
            ...
            scoring_payload = {
                ...
                "note": "参数推荐评分用于说明推荐排序；无先验数据时仅作为辅助判断，不能替代现场复核。",
            }
    ...
    def _write_branded_report_html(
            ...
            *,
            ...
            claim_boundary = str(
                ...
                or "仅作为当前数据处理状态与参数记录，不等同于 参考标注 验证。"
            )
    ...
    <head>
      ...
      <title>MyGPR 项目报告 {esc(timestamp)}</title>
      <style>
    ...
    <body>
      <main class="wrap">
        <section class="hero">
          ...
          <div>
            <h1>MyGPR 项目报告</h1>
            <div class="sub">生成时间 {esc(timestamp)} · Markdown / HTML / 600 DPI B-scan 图像</div>
          </div>
        </section>
        ...
        <section class="figure">
          <div class="label">B-scan 图像</div>
          <img src="{esc(image_name)}" alt="B-scan figure">
        </section>

        <section class="boundary">
          <strong>适用边界</strong><br>
          {esc(claim_boundary)}<br>
          <span class="label">需要人工复核: {esc(manual_review)}</span>
        </section>
