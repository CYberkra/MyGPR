...
from core.gprmax_model_inspector import GX008_MODELS_DIR, load_scene_model
...
class ResearchConsolePage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._model_scene_ids = list(self._default_model_scene_ids)
        self._build_ui()
        self.refresh()
    ...
    def _build_dashboard_page(self) -> QWidget:
        ...
        self.scene_table.setHorizontalHeaderLabels(
            ["Scene", "Soil", "Target", "Depth", "Paired", "Metrics", "AT-BG", "Backend", "尺寸"]
        )
    ...
    def _build_model_page(self) -> QWidget:
        ...
        for name, widget in [
            ("概览", self.model_overview_text),
            ("基础参数", self.model_basic_text),
            ("材料", self.model_materials_text),
            ("几何", self.model_geometry_text),
            ("扫描", self.model_scan_text),
            ("ROI", self.model_roi_text),
            ("Pair Contract", self.model_pair_text),
            ("原始/背景", self.model_diff_text),
            ("生成命令", self.model_command_text),
        ]:
    ...
    def refresh(self) -> None:
        ...
        self._model_scene_ids = self._discover_model_scene_ids()
        self._populate_dashboard()
        self._populate_at_bg()
        self._populate_evidence()
        self._populate_model_scenes()

    def _discover_model_scene_ids(self) -> list[str]:
        """Discover local GX-008 model drafts instead of showing only the initial six scenes."""
        configured = [
        ...
        local: list[str] = []
        try:
            if GX008_MODELS_DIR.exists():
                local = sorted(
                    path.name
                    for path in GX008_MODELS_DIR.iterdir()
                    if path.is_dir()
                    and (
                        (path / "raw_with_target.in").exists()
                        or (path / "background_only.in").exists()
                        or (path / "scene_manifest_draft.json").exists()
                    )
                )
        except Exception:
            local = []
        seen: set[str] = set()
        ordered: list[str] = []
        for scene_id in configured + local + self._default_model_scene_ids:
    ...
    def _format_model_geometry(model: dict[str, Any]) -> str:
        ...
        raw_targets = ResearchConsolePage._extract_target_directives(model.get("raw_text") or "")
        background_targets = ResearchConsolePage._extract_target_directives(model.get("background_text") or "")
        parts = [
            "target_design:",
            json.dumps(target, ensure_ascii=False, indent=2),
            "",
            "raw_with_target 几何目标指令:",
            "\n".join(raw_targets) if raw_targets else "未检测到 #cylinder/#sphere/#box 目标指令",
            "",
            "background_only 目标指令检查:",
            "\n".join(background_targets) if background_targets else "未检测到目标指令（符合背景配对预期）",
        ]
        return "\n".join(parts)

    @staticmethod
    def _extract_target_directives(text: str) -> list[str]:
        directives: list[str] = []
        for line in text.splitlines():
            stripped = line.strip()
            if stripped.startswith(("#cylinder:", "#sphere:")):
