...
class SidecarController:
    ...
    def _store_trace_timestamps_from_metadata(self, trace_metadata) -> None:
        ...
        if timestamps is None:
            self.host._trace_timestamps_s = None
            return
        self.host._trace_timestamps_s = np.asarray(timestamps, dtype=np.float64).copy()
    ...
    def _get_trace_timestamps_for_sidecars(self):
        ...
        return getattr(self.host, "_trace_timestamps_s", None)
    ...
    def _extract_airborne_payload_with_optional_sidecars(
        ...
        extractor = self._payload_extractor()
        try:
            return extractor(raw_data, header_info, **sidecar_kwargs)
            ...
            return extractor(raw_data, header_info)

    def _emit_sidecar_warning(self, kind: str, reason: str) -> None:
        """Allow a host-level warning override while avoiding wrapper recursion."""
        override = getattr(self.host, "__dict__", {}).get("_warn_sidecar_ignored")
        if callable(override):
            override(kind, reason)
            return
        self._warn_sidecar_ignored(kind, reason)

    def _payload_extractor(self):
        """Resolve the historical host-module loader seam used by legacy tests/tools."""
        module = sys.modules.get(self.host.__class__.__module__)
        candidate = getattr(module, "extract_airborne_csv_payload", None) if module else None
        return candidate if callable(candidate) else extract_airborne_csv_payload
